...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DEMONS:
        rt += "Requested NA to promote a Demon Disaster to Dragon."
        data['supports'].remove(user_id)
        DEMONS.remove(user_id)

    if user_id in WOLVES:
        rt += "Requested NA to promote a Wolf Disaster to Dragon."
        data['whitelists'].remove(user_id)
        WOLVES.remove(user_id)
...
def addsupport(
    ...
    if user_id in DRAGONS:
        rt += "Requested NA to demote this Dragon to Demon"
        data['sudos'].remove(user_id)
        DRAGONS.remove(user_id)
    ...
    if user_id in WOLVES:
        rt += "Requested NA to promote this Wolf Disaster to Demon"
        data['whitelists'].remove(user_id)
        WOLVES.remove(user_id)
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        message.reply_text("Requested NA to demote this user to Civilian")
        DRAGONS.remove(user_id)
        data['sudos'].remove(user_id)
